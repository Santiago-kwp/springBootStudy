## Spring Security + JWT 토큰 구현 방안 정리

https://www.notion.so/JWT-Spring-Security-myGallery-2c9a84820e1380cebe91d79a395ffb48

## **갤러리 1차 개발 내용 정리**

- 컨텐츠 선정 이유 : 웅장한 자연 암벽이나 등산, 등반 사진 등은 웹에서 쉽게 구할 수 있지만, 자신만의 독특한 경험이나, 등반 스토리가 포함된 사진 작품을 공유할 수 있는 나만의 웹서비스 구현
- 백엔드 배운 점 : 세션을 통한 로그인 및 HttpUtils 활용, 기존에 프로시저로 구현했던 1대 N 을 자바 애플리케이션에서 처리하는 점. 상황에 따른 상태코드 반환
- 프론트엔드 배운 점 : 사용자 정보를 가져오는 방법 - pinia storage를 통해 가져오고, check 를 통해 라우팅된 vue component로 이동하더라도 가져오는 법,
- 프로젝트 진행 시 중요하게 느낀점 (3가지 이상) :
  - API POSTMAN 테스트를 확실히 해서 불확실성을 완전히 제거하자
  - 설계할 때 dto 를 세심하게 정의해서 재사용성을 늘릴 수 있게 설계하자
  - 사용자 입장에서 어떤 데이터를 어떻게 보여줘야 햇갈리지 않고 이용할 수 있을지 플로우 로직 설계
  - 트랜디한 사이트를 많이 보자
  - API 설계를 해놓고 쓰자
...
![image.png](/images/backProcess.png)

1. 패키지 추가/HTTP 유틸리티 추가 &rarr; 세션 / JWT 토큰 방식
2. 상품 API 구현 : 상품 테이블 생성 &rarr; 엔티티 생성 &rarr; DTO &rarr; 리포지토리 &rarr; 서비스 &rarr; 컨트롤러 &rarr; API 테스트
3. 계정 API 구현 : 회원 테이블 &rarr; 엔티티 &rarr; DTO(회원가입, 로그인), 계정 상수 &rarr; 리포지토리 &rarr; 서비스, 헬퍼 &rarr; 계정 컨트롤러 &rarr; API 테스트
4. 장바구니 API 구현 : 장바구니 테이블 &rarr; 엔티티 &rarr; DTO &rarr; 리포지토리 &rarr; 서비스 &rarr; 컨트롤러 &rarr; API 테스트
5. 주문 API 구현 : 주문, 주문 아이템 테이블(1:N) &rarr; 엔티티 &rarr; DTO &rarr; 리포지토리 &rarr; 서비스 &rarr; 컨트롤러 &rarr; API 테스트
